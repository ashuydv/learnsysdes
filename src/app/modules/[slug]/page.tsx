...
export default async function ModulePage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      <Link href="/modules" className="text-sm text-black/50 hover:text-brand dark:text-white/50">
        ← All modules
      </Link>
      <h1 className="text-brand mt-2 text-3xl font-bold tracking-tight">{module.title}</h1>
      ...
      <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-black/10 pt-6 text-sm dark:border-white/10">
        {prevModule ? (
          <Link href={`/modules/${prevModule.slug}`} className="hover:text-brand">
            ← {prevModule.title}
          </Link>
        ) : (
          <span />
        ...
        {nextModule ? (
          <Link href={`/modules/${nextModule.slug}`} className="hover:text-brand">
            {nextModule.title} →
          </Link>
        ) : (
          <span />
